...
function nirvana_header_image() {
	...
	if (isset($himgsrc) && ($himgsrc != '')) : echo '<img id="bg_image" alt="' . get_bloginfo( 'name' ) . '" title="' . get_bloginfo( 'name' ) . '" src="' . esc_url( $himgsrc ) . '"  />';  endif;
...
if ( ! function_exists( 'nirvana_skiplink' ) ):
function nirvana_skiplink() {
	?>
		<a class="skip-link screen-reader-text" href="#main" title="<?php esc_attr_e( 'Skip to content', 'nirvana' ); ?>"> <?php _e( 'Skip to content', 'nirvana' ); ?> </a>
	<?php
}// nirvana_skiplink()
endif;

add_action( 'cryout_body_hook', 'nirvana_skiplink', 2 );
	...
	for ($i=1; $i<=9; $i+=2) {
		...
		if ( ${"nirvana_social$j"} ) {
			...
			class="socialicons social-<?php echo esc_attr(${"nirvana_social$i"}); ?>" title="<?php echo ${"nirvana_social_title$i"} !="" ? esc_attr(${"nirvana_social_title$i"}) : esc_attr(${"nirvana_social$i"}); ?>">
				<img alt="<?php echo esc_attr(${"nirvana_social$i"}); ?>" src="<?php echo esc_url( get_template_directory_uri() ) . '/images/socials/'.${"nirvana_social$i"}.'.png'; ?>" />
			</a><?php
		}
	}
	echo '</div>';
